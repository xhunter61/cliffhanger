Template.bookprofile.helpers({
  ...
  'butterID': function(){
    var rootChapter= Butterfly.findOne({bookid:Router.current().params._id,chapternumber:0});
        if(rootChapter){
  ...
  'isButterfly': function() {
      var butterfly= Booklist.find({_id:Router.current().params._id,butterfly:true}, { sort: {timestamp: -1}, limit: 50 }).count();
 ...
 'bookEntity': function() {
  ...
  'bookChapters': function() {
    return Tweets.find({bookid:Router.current().params._id}, { sort: {timestamp: -1}, limit: 2500 });
  },
